...
export default function RecommendationBlogs({ posts, loading }: { posts?: Post[]; loading?: boolean }) {
  const [currentIndex, setCurrentIndex] = useState(0);

  // map API posts to the BlogPost props shape
  const apiBase = import.meta.env.VITE_API_BASE || '';
  const mappedPosts = (posts && posts.length > 0)
    ? posts.map((p) => ({
      id: p.id,
  ...
  const dataSource = mappedPosts;

  const handlePrev = () => {
    setCurrentIndex((prev) => (prev === 0 ? Math.max(0, dataSource.length - 3) : prev - 1));
  };

  const handleNext = () => {
    setCurrentIndex((prev) => (prev >= Math.max(0, dataSource.length - 3) ? 0 : prev + 1));
  };

  const visibleBlogs = dataSource.slice(currentIndex, currentIndex + 3);
    ...
    return (
      <section id="recommendations" className="py-20 bg-white">
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          <Headings
            upheading="Handpicked For You"
            mainHeading="Recommended Adventures"
            highlightWords={['Recommended']}
            subheading="Curated stories based on popular choices and trending destinations"
          />
        </div>
  ...
  return (
    <section id="recommendations" className="py-20 bg-white">
      ...
      {/* full-width wrapper so the posts grid centers relative to the viewport */}
      <div className="w-full">
